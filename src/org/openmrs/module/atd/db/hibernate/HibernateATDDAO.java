...
public class HibernateATDDAO implements ATDDAO
{
	...
	public FormInstance addFormInstance(Integer formId)
	{
		try
		{
			...
			Connection con = this.sessionFactory.getCurrentSession()
					.connection();
			int rowsInserted = 0;
			String sql = "insert into atd_form_instance(form_instance_id,form_id) "
					+ "select max(form_instance_id)+1,? from atd_form_instance "
					+ "where form_id in (select form_id from form where name=?)";
			try
			{
			...
			if (rowsInserted > 0)
			{
				sql = "select max(form_instance_id) as form_instance_id,"
						+ "form_id from atd_form_instance where form_id=?"
						+ " group by form_id";
	...
	public PatientATD getPatientATD(int formInstanceId, int fieldId, int formId)
	{
		try
		{
			...
			String sql = "select * from atd_patient_atd "
					+ "where form_instance_id=? and field_id=? and form_id=?";
			SQLQuery qry = this.sessionFactory.getCurrentSession()
					.createSQLQuery(sql);
			qry.setInteger(0, formInstanceId);
			qry.setInteger(1, fieldId);
			qry.setInteger(2, formId);
			...
			String formAttributeName)
	{
		try
		{
			...
			if (formAttribute != null)
			{
				...
				String sql = "select * from atd_form_attribute_value where form_id=? and form_attribute_id=?";
				SQLQuery qry = this.sessionFactory.getCurrentSession()
						.createSQLQuery(sql);
	...
	public Session getSessionByEncounter(int encounterId)
	{
		try
		{
			String sql = "select max(session_id) as session_id,encounter_id "+
			"from atd_session where encounter_id=? group by encounter_id";
			SQLQuery qry = this.sessionFactory.getCurrentSession()
					.createSQLQuery(sql);
	...
	public PatientState getPatientStateByFormInstanceAction(Integer formId,
			Integer formInstanceId, String action)
	{

		try
		{
			// limit to states for the session that match the form id
			String sql = "select * from atd_patient_state where form_instance_id=? and form_id=? and retired=? order by start_time desc, end_time desc";
			SQLQuery qry = this.sessionFactory.getCurrentSession()
					.createSQLQuery(sql);
			qry.setInteger(0, formInstanceId);
			qry.setInteger(1, formId);
			...
			String stateName, Date optionalDateRestriction)
	{
		try
		{
			...
			String sql = "select * from atd_patient_state where state in "
					+ "(?) and end_time is null and retired=?" + dateRestriction
	...
	public List<PatientState> getUnfinishedPatientStatesAllPatients(Date optionalDateRestriction)
	{
		try
		{
			...
			String sql = "select * from atd_patient_state where end_time is null and retired=?"
			...
			if (optionalDateRestriction != null)
			{
				qry.setDate(1, optionalDateRestriction);
			}
			return qry.list();
		} catch (Exception e)
		{
	...
	private List<String> getListMappedStates(Integer programId){
		...
		String firstState = adminService.getGlobalProperty("atd.initialState");
		StateMapping mapping = null;
		
		if(firstState != null){
			orderedStateNames.add(firstState);
			mapping = stateMap.get(firstState);
			
			while(mapping != null){
				firstState = mapping.getNextState().getName();
				orderedStateNames.add(firstState);
				mapping = stateMap.get(firstState);
			}
		}
			...
			Date optionalDateRestriction,Integer programId)
	{
		try
		{
			...
			String sql = "select * from atd_patient_state aps, atd_session atds, encounter e" +
			" where aps.session_id = atds.session_id and atds.encounter_id = e.encounter_id " +
			" and retired=? "+dateRestriction+" order by e.encounter_datetime desc,start_time desc";
			...
			List<String> mappedStateNames = getListMappedStates(programId);
	...
	public List<Integer> getFormInstancesByEncounterId(String formName, Integer encounter_id){
		...
		String sql = "select distinct form_instance_id from atd_patient_state atdps " 
