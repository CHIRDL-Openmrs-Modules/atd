			...
			Map<String, Object> parameters) throws LogicException
	{
		...
		try
		{
			...
			for (Concept currConcept : concepts)
			{
				if (criteria == null)
				{
					criteria = new LogicCriteria(currConcept
							.getName().getName());
				} else
				{
					criteria = criteria.or(new LogicCriteria(currConcept
							.getName().getName()));
				}
			}
			...
			if (criteria != null)
			{
				...
				criteria = criteria.and(
						new LogicCriteria(Operator.CONTAINS, answer)).last();
			}

			ruleResult = context.read(patient, context
